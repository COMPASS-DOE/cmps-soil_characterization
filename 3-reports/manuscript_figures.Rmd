...
#corr_wle = fit_correlations_function(dat = data_wide_PCA %>% filter(region == "WLE"), TITLE = "WLE" )
#corr_cb = fit_correlations_function(dat = data_wide_PCA %>% filter(region == "CB"), TITLE = "CB")
...
```{r v2-pca_gg, fig.width=10, fig.height=6}
## PCA plots overall
#gg_pca_overall = 
...
gg_pca_wle = 
  ...
  scale_color_manual(breaks = c("upland", "transition", "wte", "wc"), 
                     values = pal_transect)+
  scale_fill_manual(breaks = c("upland", "transition", "wte", "wc"), 
                    values = pal_transect)+
...
```{r v2-scaled_gg2, fig.height = 8, fig.width = 8}
data_long_all = 
  data_combined_wide %>% 
...
```{r v2-scaled_gg2_split, fig.height = 10, fig.width = 14}
library(ggh4x) # for nested facets
# https://teunbrand.github.io/ggh4x/reference/facet_nested.html
...
```{r v2-scaled_gg2_split-NEW, fig.height = 10, fig.width = 14}
library(ggh4x) # for nested facets
# https://teunbrand.github.io/ggh4x/reference/facet_nested.html

data_scaled_NEW2 = 
  data_scaled_NEW %>% 
  filter(name %in% c("WEOC", "TS", "Na", "Chloride", "Fe", "P")) %>% 
    mutate(name = factor(name, levels = c("TC", "TS", "WEOC",
                                        "Ca", "Na", "Chloride",
                                        "NH4N", "Fe", "P"))) 
...
```{r v2-scaled_gg2_gwi, fig.height = 8, fig.width = 8}
data_scaled_NEW %>% 
  filter(site == "GWI") %>% 
  ggplot(aes(x = transect, y = value_scaled, group = site, color = site))+
  geom_line(linewidth = 1) + geom_point(size = 2)+
...
plot_transect_as_x = function(data, YLAB = "", TITLE = "", SUBTITLE = "", SCALES = "free_x"){
  ...
  data = data %>% mutate(transect = factor(transect, levels = c("upland", "transition", "wc"))) %>% 
    filter(!is.na(value))
...
```{r analytesx-4, fig.height = 15, fig.width = 8}
...
gg_spc =
  data_combined_subset %>% filter(grepl("spCond", name)) %>% filter(region == "CB") %>% 
  ...
  #  facet_wrap(~region, labeller = as_labeller(c("CB" = "Chesapeake Bay", "WLE" = "Lake Erie")))+
    theme_kp()+
    labs(x = "",
...
# plot Chloride
gg_cl = 
  data_combined_subset %>% filter(grepl("Chloride", name)) %>% filter(region == "CB") %>% 
  ggplot(aes(x = transect, y = value)) +
    # plot points
    geom_point(aes(group = site, color = site),
               size = 2.5, stroke = 1, position = position_dodge(width = 0.4))+
