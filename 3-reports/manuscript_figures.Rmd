...
```{r map-poster, fig.height = 9.5, fig.width = 13}
...
gg_pca_wle = 
  ...
  scale_color_manual(breaks = c("upland", "transition", "wte", "wetland"), 
                     values = pal_transect)+
  scale_fill_manual(breaks = c("upland", "transition", "wte", "wetland"), 
                    values = pal_transect)+
...
```{r v2-pca_gg_regions-poster, fig.width=7, fig.height=7}
...
```{r v2-scaled_gg2_split-NEW, fig.height = 10, fig.width = 12}
library(ggh4x) # for nested facets
# https://teunbrand.github.io/ggh4x/reference/facet_nested.html
...
plot_transect_as_x = function(data, YLAB = "", TITLE = "", SUBTITLE = "", SCALES = "free_x"){
  ...
  data = data %>% mutate(transect = factor(transect, levels = c("upland", "transition", "wetland"))) %>% 
    filter(!is.na(value))
...
elevation_processed = 
  elevation %>% 
  mutate(transect = factor(transect, levels = c("water edge", "wetland", "transition", "upland"))) %>% 
  reorder_site()
...
gg_wle = 
  ...
  scale_color_manual(values = c("grey", "#2E5894", "#FFB200", "#96001B"),
                     breaks = c("water edge", "wetland", "transition", "upland"))+
  facet_nested_wrap(region ~ site)+
  labs(x = "",
       y = "Elevation, m
       ",
